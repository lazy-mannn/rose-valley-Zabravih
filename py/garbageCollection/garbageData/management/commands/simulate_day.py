...
class Command(BaseCommand):
    help = "Simulate 1 day passing - bins fill up gradually"
    ...
    def handle(self, *args, **options):
        ...
        stats = {'collected': 0, 'overflow': 0, 'high': 0, 'normal': 0}
        
        for can in TrashCan.objects.all():
            current = can.get_predicted_fill_level()
            rate = can.get_average_daily_fill_rate()
            
            # Calculate fill after X days
            increase = rate * days * random.uniform(0.85, 1.15)  # ±15% variance
        ...
        self.stdout.write(f"   ⚠️  High (≥70%): {stats['high']}")
